...
#define CHECK_DOUBLE(actual, expected)                                                \
  {                                                                                   \
    if (!vtkAddonTestingUtilities::Check<double>(                                     \
          __LINE__, #actual " != " #expected, (actual), (expected), "CheckDouble")) { \
      return EXIT_FAILURE;                                                            \
    }                                                                                 \
  }
...
#define CHECK_DOUBLE_TOLERANCE(actual, expected, tolerance)                                                        \
  {                                                                                                                \
    if (!vtkAddonTestingUtilities::CheckDoubleTolerance(                                                           \
          __LINE__, #actual " != " #expected " (tolerance: " #tolerance ")", (actual), (expected), (tolerance))) { \
      return EXIT_FAILURE;                                                                                         \
    }                                                                                                              \
  }
...
#define CHECK_BOOL(actual, expected)                                                   \
  {                                                                                    \
    if (!vtkAddonTestingUtilities::CheckInt(                                           \
          __LINE__, #actual " != " #expected, (actual) ? 1 : 0, (expected) ? 1 : 0)) { \
      return EXIT_FAILURE;                                                             \
    }                                                                                  \
  }
